...
class ControllerDocs
{
    ...
    {
        $carros = $this->database->getData("SELECT `ID`, `Nome` FROM `fctDocumentos` order by `Nome`");
    ...
    {
        $p['id'] = $id;
        $carro = $this->database->getData("SELECT `ID`, `Nome`, `Texto`, `Cabecalho`, `Rodape` FROM `fctDocumentos` WHERE id = :id", $p);
        //print_r($carro);
        if ($carro) {
