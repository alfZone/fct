...
$table->setFieldList("idTurma", 1, "SELECT fctTurmas.`ID`, concat(fctAnosletivos.Anoletivo, ' - ', `NomeTurma`) as Turma FROM `fctTurmas` inner join fctAnosletivos on fctAnosletivos.ID=fctTurmas.IDAnoLetivo where fctAnosletivos.Ativo=1 order by `NomeTurma`;");
