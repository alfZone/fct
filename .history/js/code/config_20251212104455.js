/**
 * @author alf
 * @copyright 2022
 * @ver 2.0
 */

class Config {
  constructor() {}

  color(pos) {
    let cores = ["bg-info", "bg-primary", "bg-success", "bg-warning", "bg-danger", "bg-black", "bg-indigo", "bg-fuchsia", "bg-navy", "bg-info", "bg-primary", "bg-success", "bg-warning", "bg-danger", "bg-black", "bg-indigo", "bg-fuchsia", "bg-navy"];
    if (pos > cores.length) {
      pos = 0;
    }
    return cores[pos];
  }

  get url() {
    return window.location.protocol + "//" + window.location.host + "/";
  }

  get urlRenderAutentication() {
    console.log(this.url + "/fct/public/autenticacao/getAutentication");
    return this.url + "/fct/public/autenticacao/getAutentication";
  }

  get urlLogout() {
    return this.url + "/fct/public/autenticacao/logout";
  }

  get urlLogin() {
    return this.url + "/fct/public/admin/login";
  }
}
