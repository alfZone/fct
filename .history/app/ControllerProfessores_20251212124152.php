...
class ControllerProfessores {
    ...
    public function update() {
        ...
        $p['DataNascimento']=$putData['DataNascimento'];
        $p['CC']=$putData['CC'];
        $p['Contribuinte']=$putData['Contribuinte'];
        $p['NomeEE']=$putData['NomeEE'];
        $p['ContactoEE']=$putData['ContactoEE'];
        $p['IDturma']=$putData['IDturma'];

        $resp = $this->database->setData("UPDATE fctProfessores SET marca = :marca, detalhes = :detalhes, foto = :foto WHERE id = :id", $p);
        echo json_encode($resp, JSON_UNESCAPED_UNICODE);
    }
