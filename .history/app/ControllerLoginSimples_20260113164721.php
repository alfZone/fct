...
class ControllerLoginSimples
{
  ...
  public function getUserByUserPass($user, $passw, $json = true)
  {
    $p['user'] = $user;
    $p['passw'] = md5($passw);
    $user = $this->database->getData("SELECT fctEmpresasCurso.`NIF` as id, fctEmpresasCurso.`NIF` as user, fctEmpresas.NomeEmpresa, fctEmpresas.Email FROM `fctEmpresasCurso` inner join fctEmpresas on fctEmpresasCurso.NIF=fctEmpresas.NIF WHERE user=:user and passw=:passw", $p);
    if ($json) {
  ...
  public function validaLogin()
  {
    ...
    if (!isset($_POST['user']) || !isset($_POST['passw1'])) {
      header('location: ' . _CAMINHO_LOGIN . '?e=400');
      exit;
    } else {
          $user = $this->getUserByUserPass($_POST['user'], $_POST['passw1'], false);
