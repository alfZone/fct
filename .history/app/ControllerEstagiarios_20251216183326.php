...
class ControllerEstagiarios {
    ...
    public function getAll() {
        $estagi = $this->database->getData("SELECT fctEstagiarios.`ID`, fctEstagiarios.`NIF`, fctEmpresas.NomeEmpresa, fctEstagiarios.`Processo`, fctAlunos.Nome, fctAlunos.IDturma, fctTurmas.NomeTurma, fctTurmas.IDcurso, fctEstagiarios.`IDAnoLetivo`, fctAnosletivos.Anoletivo, `ProcessoProf`, fctProfessores.Nome FROM `fctEstagiarios` inner join fctAlunos on fctAlunos.Processo=fctEstagiarios.Processo inner join fctEmpresas on fctEmpresas.NIF=fctEstagiarios.NIF inner join fctProfessores on fctProfessores.Processo=fctEstagiarios.ProcessoProf inner join fctAnosletivos on fctAnosletivos.ID=fctEstagiarios.IDAnoLetivo INNER JOIN fctTurmas on fctTurmas.ID=fctAlunos.IDturma;");
    ...
    public function getById($id) {
        $p['id']=$id;
        $estagi = $this->database->getData("SELECT * FROM fctEstagiarios WHERE id = :id", $p);
        //print_r($estagiarios);
        if ($estagi) {
            echo json_encode($estagi, JSON_UNESCAPED_UNICODE);
        } else {
