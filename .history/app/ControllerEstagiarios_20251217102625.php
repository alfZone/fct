...
class ControllerEstagiarios
{
    ...
    public function createEstagiarios($curso)
    {
        $p['curso'] = $curso;

        $aluno = $this->database->getData("SELECT `Processo`, fctAlunos.Nome, fctTurmas.IDcurso as Curso, fctTurmas.IDAnoLetivo, fctTurmas.NomeTurma, fctAlunos.IDturma FROM `fctAlunos` inner join fctTurmas on fctTurmas.id=IDturma INNER join fctAnosletivos on fctAnosletivos.ID= fctTurmas.IDAnoLetivo WHERE fctTurmas.IDcurso = 5 ORDER BY fctTurmas.NomeTurma, Nome;", $p);
