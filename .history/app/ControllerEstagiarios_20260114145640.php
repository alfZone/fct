...
class ControllerEstagiarios
{
    ...
    public function inscreverMatriculadosParaEstagio()
    {
        ...
        $estagi = $this->database->getData("INSERT INTO `fctEstagiarios` (`ID`, `NIF`, `Processo`, `IDAnoLetivo`, `ProcessoProf`)
SELECT 
    NULL, -- ou um valor para ID se for auto_increment
    '',   -- NIF vazio ou valor padrão
    fctMatricula.`processo`,
    fctTurmas.IDAnoLetivo,
    ''    -- ProcessoProf vazio ou valor padrão
FROM `fctMatricula` 
...
WHERE fctAnosletivos.Ativo = 1;", $p);
        //print_r($estagi);
...
/* INSERT INTO `fctEstagiarios` (`ID`, `NIF`, `Processo`, `IDAnoLetivo`, `ProcessoProf`)
SELECT 
    NULL, -- ou um valor para ID se for auto_increment
    '',   -- NIF vazio ou valor padrão
    fctMatricula.`processo`,
...
WHERE fctAnosletivos.Ativo = 1; */
    ...
    public function getAllporAnoAtivoProfessor($prof)
    {
        ...
        $estagi = $this->database->getData("SELECT fctEstagiarios.`ID`, fctEstagiarios.`NIF`, fctEmpresas.NomeEmpresa, fctEstagiarios.`Processo`, fctAlunos.Nome, 
                                                fctAlunos.IDturma, fctTurmas.NomeTurma, fctTurmas.IDcurso, fctEstagiarios.`IDAnoLetivo`, fctAnosletivos.Anoletivo, 
                                                `ProcessoProf`, fctProfessores.Nome as Professor, fctAnosletivos.Ativo, fctAlunos.Contacto FROM `fctEstagiarios` 
                                                inner join fctAlunos on fctAlunos.Processo=fctEstagiarios.Processo 
                                                inner join fctEmpresas on fctEmpresas.NIF=fctEstagiarios.NIF 
                                                inner join fctProfessores on fctProfessores.Processo=fctEstagiarios.ProcessoProf 
                                                inner join fctAnosletivos on fctAnosletivos.ID=fctEstagiarios.IDAnoLetivo 
                                                INNER JOIN fctTurmas on fctTurmas.ID=fctAlunos.IDturma
                                                WHERE fctAnosletivos.Ativo=1 and ProcessoProf = :prof
                                                order by fctTurmas.NomeTurma, fctAlunos.Nome ;", $p);
        //print_r($estagi);
    ...
    // Obter os estagiarios por ano letivo e curso
    public function getAllVagasCursoAnoAtivo($curso)
    {
        $p['curso'] = $curso;
        $estagi = $this->database->getData("SELECT fctVagasAnoEmpresa.`id`, fctVagasAnoEmpresa.`NIF`, fctEmpresas.NomeEmpresa, `IdCurso`, fctCursos.NomeCurto, `IDAnoLetivo`, 
                                                fctAnosletivos.Anoletivo, `NumVagas`, `Obs` FROM `fctVagasAnoEmpresa` 
                                                inner join fctEmpresas on fctEmpresas.NIF=fctVagasAnoEmpresa.NIF 
                                                inner join fctCursos on fctCursos.ID=fctVagasAnoEmpresa.IdCurso 
                                                inner join fctAnosletivos on fctAnosletivos.ID=fctVagasAnoEmpresa.IDAnoLetivo 
                                                where fctAnosletivos.Ativo=1 and fctVagasAnoEmpresa.IdCurso=:curso order by fctEmpresas.NomeEmpresa;", $p);
        //print_r($estagi);

        echo json_encode($estagi, JSON_UNESCAPED_UNICODE);
    }

    // Obter os estagiarios por ano letivo e curso e empresa
    public function getAllVagasCursoNifAnoAtivo($curso, $nif)
    {
        $p['curso'] = $curso;
        $p['nif'] = $nif;
        $estagi = $this->database->getData("SELECT fctVagasAnoEmpresa.`id`, fctVagasAnoEmpresa.`NIF`, fctEmpresas.NomeEmpresa, `IdCurso`, fctCursos.NomeCurto, `IDAnoLetivo`, 
                                                ...
                                                inner join fctCursos on fctCursos.ID=fctVagasAnoEmpresa.IdCurso 
                                                inner join fctAnosletivos on fctAnosletivos.ID=fctVagasAnoEmpresa.IDAnoLetivo 
                                                where fctAnosletivos.Ativo=1 and fctVagasAnoEmpresa.IdCurso=:curso and fctVagasAnoEmpresa.`NIF`=:nif 
                                                order by fctEmpresas.NomeEmpresa;", $p);
        //print_r($estagi);
    ...
    // Obter os estagiarios por ano letivo e curso e empresa
    public function getAllVagasNifAnoAtivo($nif)
    {
        $p['nif'] = $nif;
        $estagi = $this->database->getData("SELECT fctVagasAnoEmpresa.`id`, fctVagasAnoEmpresa.`NIF`, fctEmpresas.NomeEmpresa, `IdCurso`, fctCursos.NomeCurto, `IDAnoLetivo`, 
                                                fctAnosletivos.Anoletivo, `NumVagas`, `Obs` FROM `fctVagasAnoEmpresa` 
                                                ...
                                                inner join fctCursos on fctCursos.ID=fctVagasAnoEmpresa.IdCurso 
                                                inner join fctAnosletivos on fctAnosletivos.ID=fctVagasAnoEmpresa.IDAnoLetivo 
                                                where fctAnosletivos.Ativo=1 and fctVagasAnoEmpresa.`NIF`=:nif 
                                                order by fctEmpresas.NomeEmpresa;", $p);
        //print_r($estagi);
    ...
    public function atualizarVagasObs()
    {
        ...
        $p['NumVagas'] = $putData['NumVagas'];
        $p['Obs'] = $putData['Obs'];
        $p['IdCurso'] = $putData['IdCurso'];
        $p['IDAnoLetivo'] = $putData['IDAnoLetivo'];
        $p['NIF'] = $putData['NIF'];

        $resp = $this->database->setData("INSERT INTO fctVagasAnoEmpresa (NIF, IdCurso, IDAnoLetivo, NumVagas, Obs) 
                                            VALUES (:NIF, :IdCurso, :IDAnoLetivo, :NumVagas, :Obs)
                                            ON DUPLICATE KEY UPDATE NumVagas = VALUES(NumVagas), Obs = VALUES(Obs);", $p);
        echo json_encode($resp, JSON_UNESCAPED_UNICODE);
    }
