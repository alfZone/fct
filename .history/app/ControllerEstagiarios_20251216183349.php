...
class ControllerEstagiarios {
    ...
    public function getAll() {
        $estagi = $this->database->getData("SELECT fctEstagiarios.`ID`, fctEstagiarios.`NIF`, fctEmpresas.NomeEmpresa, fctEstagiarios.`Processo`, fctAlunos.Nome, 
                                                ...
                                                inner join fctEmpresas on fctEmpresas.NIF=fctEstagiarios.NIF inner join fctProfessores on fctProfessores.Processo=fctEstagiarios.ProcessoProf inner join fctAnosletivos on fctAnosletivos.ID=fctEstagiarios.IDAnoLetivo INNER JOIN fctTurmas on fctTurmas.ID=fctAlunos.IDturma;");
