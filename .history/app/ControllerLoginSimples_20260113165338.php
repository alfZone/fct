...
class ControllerLoginSimples
{
  ...
  public function getUserByUserPass($user, $passw, $json = true)
  {
    ...
    $p['passw'] = md5($passw);
    print_r($p);
    $user = $this->database->getData("SELECT fctEmpresasCurso.`NIF` as id, fctEmpresasCurso.`NIF` as userx, fctEmpresas.NomeEmpresa, fctEmpresas.Email FROM `fctEmpresasCurso` inner join fctEmpresas on fctEmpresasCurso.NIF=fctEmpresas.NIF WHERE user=:user and passw=:passw", $p);
    if ($json) {
