...
class ControllerEmpresas
{
    ...
    {
        $p['NIF'] = $NIF;
        $empresas = $this->database->getData("SELECT fctEmpresasCurso.`id`, `IdCurso`, `NIF`, fctCursos.Nome FROM `fctEmpresasCurso` inner join fctCursos on fctCursos.ID=fctEmpresasCurso.IdCurso WHERE NIF = :NIF", $p);
        //print_r($empresas);
        if ($empresas) {
            echo json_encode($empresas, JSON_UNESCAPED_UNICODE);
        } else {
