...
class ControllerEstagiarios
{
    ...
    public function atualizarVagasObs()
    {
        ...
        $p['obs'] = $putData['obs'];
        $p['curso'] = $putData['curso'];
        $p['idAnoLetivo'] = $putData['idAnoLetivo'];
        $p['NIF'] = $putData['NIF'];

        $resp = $this->database->setData("UPDATE fctEstagiarios SET numVagas = :numVagas, obs = :obs WHERE curso = :curso AND idAnoLetivo = :idAnoLetivo AND NIF = :NIF", $p);
        echo json_encode($resp, JSON_UNESCAPED_UNICODE);
    }
