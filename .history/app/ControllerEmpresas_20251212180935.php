...
class ControllerEmpresas
{
    ...
    public function update()
    {
        ...
        $resp = $this->database->setData("UPDATE fctEmpresas SET `Empresa`=:NomeEmpresa,`Cargo`=:Cargo,`Morada`=:MoradaSede,`Contacto_Pessoa`=:ResponsavelEstagiario,
                                            `Horario`=:HorarioFuncionamento,`Duracao`=:HorasDia,`Nome_Contacto`=:Monitor,`Cargo_Contacto`=:CargoMonitor,`Telefone_Email`=:Email,`Observacoes`=:Observacoes WHERE NIF = :NIF", $p);
        echo json_encode($resp, JSON_UNESCAPED_UNICODE);
    }
