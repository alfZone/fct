...
class ControllerEmpresas
{
    ...
    {
        $p['NIF'] = $NIF;
        $empresas = $this->database->getData("SELECT fctEmpresasCurso.`id`, `IdCurso`, `NIF`, fctCursos.Nome FROM `fctEmpresasCurso` inner join fctCursos on fctCursos.ID=fctEmpresasCurso.IdCurso WHERE NIF = :NIF order by fctCursos.Nome;", $p);
        //print_r($empresas);
        if ($empresas) {
            echo json_encode($empresas, JSON_UNESCAPED_UNICODE);
        } else {
    ...
    public function update()
    {
        ...
        $resp = $this->database->setData("UPDATE `fctEmpresas` SET `NomeEmpresa`=:NomeEmpresa,`Cargo`=:Cargo,`ResponsavelEstagiario`=:ResponsavelEstagiario,
                                            ...
                                            `Monitor`=:Monitor,`CargoMonitor`=:CargoMonitor,`Observacoes`=:Observacoes,`Email`=:Email,`Telefone`=:Telefone
                                            WHERE NIF = :NIF", $p);
        echo json_encode($resp, JSON_UNESCAPED_UNICODE);
    }
