...
class ControllerLoginSimples
{
  ...
  public function getUserByUserPass($user, $passw, $json = true)
  {
    $p['user'] = $user;
    $p['passw'] = $passw;
    $user = $this->database->getData("SELECT id, `Nome` as name, `Email` as email, user as user, `Telefone`, `DC` as type, `ativo` FROM `fctProfessores` WHERE user=:user and ativo=1", $p);
    if ($json) {
