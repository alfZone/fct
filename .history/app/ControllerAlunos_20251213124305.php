...
class ControllerAlunos
{
    ...
    public function getByCurso($curso)
    {
        ...
        `ContactoEE`, `IDturma`, fctTurmas.NomeTurma, fctTurmas.IDcurso as Curso, fctTurmas.IDAnoLetivo FROM `fctAlunos` inner join fctTurmas on fctTurmas.id=IDturma  WHERE Curso = :curso", $p);
        //print_r($aluno);
        if ($aluno) {
            echo json_encode($aluno, JSON_UNESCAPED_UNICODE);
        } else {
