...
<body>
  ...
  <script>
    ...
    const readAPICursosAtivos = async () => {
      ...
      if (dataArray[0].numElemtnos == 0) {
        document.getElementById("cursos").innerHTML = "Sem cursos associados";
        document.getElementById("vagas").innerHTML = "Sem vagas associadas";
        return;
      }
      for (const dataRecord of dataArray) {
        //console.log(dataRecord.IdCurso)
        document.getElementById("anoLetivo").innerHTML = dataRecord.IDAnoLetivo;
        resp = await readAPIVagarCurso(dataRecord.IdCurso);
        console.log(resp)
