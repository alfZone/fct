  ...
  <script>
    ...
    const readAPICursosAtivos = async () => {
      ...
      for (const dataRecord of dataArray) {
        ...
        strHtml2 += `
          <div class="col-md-6">
            <label class="form-label">Vagas de FCT para ${dataRecord.Nome}</label>
            <input type="text" class="form-control" id="vfct${dataRecord.ID}" maxlength="9">
          </div>
           `
      }
      document.getElementById("cursos").innerHTML = strHtml;
      document.getElementById("vagas").innerHTML = strHtml;
    }
    // function call
