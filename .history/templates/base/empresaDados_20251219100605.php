...
<body>
  ...
  <script>
      ...
      for (const dataRecord of dataArray) {
        ...
        strHtml2 += `
          <div class="col-md-6">
            ...
            <input type="number" class="form-control" id="vfct${dataRecord.ID}" maxlength="9">
          </div>
          <div class="col-md-6">
            <label class="form-label">Observações para ${dataRecord.NomeCurto}</label>
           <textarea class="form-control" rows="3" id="obs${dataRecord.ID}" name="obs"></textarea>
          </div>
           `
      }
    ...
    const readAPIVagarCurso = async (curso) => {
      const response = await fetch("https://turma12r.alunos.esmonserrate.org/fct/public/api/estagiarios/vagas/"+curso)
      const dataArray = await response.json()
      for (const dataRecord of dataArray) {
        document.getElementById("vfct"+curso).value = dataRecord.VagasFCT;
        document.getElementById("obs"+curso).value = dataRecord.Observacoes;
      }
      document.getElementById("cursos").innerHTML = strHtml;
      document.getElementById("vagas").innerHTML = strHtml2;
    }
