...
<body>
  ...
  <script>
    ...
    const readAPICursosAtivos = async () => {
      ...
      const dataArray = await response.json()
      for (const dataRecord of dataArray) {
        resp = await readAPIVagarCurso(dataRecord.ID);
        console.log(resp)
        strHtml += `${dataRecord.Nome}|`
        strHtml2 += `
          <div class="col-md-6">
            <label class="form-label">Vagas de FCT para ${dataRecord.NomeCurto}</label>
            <input type="number" class="form-control" id="vfct${dataRecord.ID}" maxlength="9" val="${resp[0]}">
          </div>
          <div class="col-md-6">
            <label class="form-label">Observações para ${dataRecord.NomeCurto}</label>
           <textarea class="form-control" rows="3" id="obs${dataRecord.ID}" name="obs">${resp[1]}</textarea>
          </div>
           `
      }
    ...
    const readAPIVagarCurso = async (curso) => {
      ...
      for (const dataRecord of dataArray) {
        ...
        console.log(dataRecord.NumVagas);
