...
$table->setFieldList("idTurma", 1, "SELECT fctTurmas.`ID`, concat(fctAnosletivos.Anoletivo, ' - ', `NomeTurma`) as Turma FROM `fctTurmas` 
                                        inner join fctAnosletivos on fctAnosletivos.ID=fctTurmas.IDAnoLetivo order by `NomeTurma`;");
$table->setFieldList("processo", 1, "SELECT fctAlunos.`Processo`, fctAlunos.Nome FROM `fctAlunos` order by fctAlunos.Nome;");
...
$table->setCriterio("idTurma in (select ID from fctTurmas where IDAnoLetivo=(select ID from fctAnosletivos where Ativo=1))");
